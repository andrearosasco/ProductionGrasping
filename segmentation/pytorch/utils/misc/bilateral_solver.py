...
grid_params = {
    ...
    'sigma_spatial': 2  # Spatial bandwidth
}
...
def apply_bilateral(reference, target, confidence, thresh=0.7, plot=False):
    ...
    # cv2.imshow('tc_filt', cv2.applyColorMap(cv2.convertScaleAbs(tc_filt.reshape(256, 192), alpha=0.03), cv2.COLORMAP_JET))
    # cv2.imshow('c_filt', cv2.applyColorMap(cv2.convertScaleAbs(c_filt.reshape(256, 192), alpha=0.03), cv2.COLORMAP_JET))
    # cv2.imshow('score', cv2.applyColorMap(cv2.convertScaleAbs(confidence, alpha=0.03), cv2.COLORMAP_JET))
    # cv2.imshow('score_scaled', cv2.applyColorMap(cv2.convertScaleAbs(c.reshape(256, 192), alpha=0.03), cv2.COLORMAP_JET))
