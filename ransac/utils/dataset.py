...
class TestSet(data.Dataset):
    def __init__(self, config, box_width, box_height, box_depth, rotations):
        #  Backbone Input
    ...
    def __getitem__(self, idx):  # Must return complete, imp_x and impl_y
        ...
        while True:
            mesh = o3d.geometry.TriangleMesh.create_box(width=self.box_width, height=self.box_height,
                                                        depth=self.box_depth)
            mesh.translate(-mesh.get_center())

            mesh = mesh.rotate(self.rotations[idx].as_matrix())
            ...
            # draw_geometries([mesh, o3d.geometry.TriangleMesh.create_coordinate_frame(origin=[0, 0, c_dist])])
